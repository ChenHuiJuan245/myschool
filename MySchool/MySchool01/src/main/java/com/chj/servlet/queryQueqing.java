 ...
 */
@WebServlet(urlPatterns = {"/queryQueqing.do"})
public class queryQueqing extends HttpServlet {
    @Override
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        if (null==method||!method.equals("queryid")){
            ...
            req.getRequestDispatcher("/view/showQueQing.jsp").forward(req,resp);
        }else {
            String id=req.getParameter("id");
            QueqingEntity qu=service.queryById(Integer.parseInt(id));
            req.setAttribute("qu",qu);
            req.getRequestDispatcher("updatequeqing.do").forward(req,resp);
        }
    }
}
